	...
	<table
		...
		<thead>
			<tr>
				...
				<th style="text-align: right; font-weight: 400;">${order.orderDate }</th>
			</tr>
		</thead>
		<tbody>
			<tr>
				...
				<td colspan="2" style="border: solid 1px #ddd; padding: 10px 20px;">
					<p style="font-size: 14px; margin: 0 0 6px 0;">
						<span
							...
							status</span><b style="color: green; font-weight: normal; margin: 0">${order.status ? 'Transport' : 'Pending...' }</b>
					</p>
					<p style="font-size: 14px; margin: 0 0 6px 0;">
						<span
					...
					<p style="font-size: 14px; margin: 0 0 0 0;">
						<span
							style="font-weight: bold; display: inline-block; min-width: 146px">Created
							Date</span> ${order.orderDate }
					</p>
				</td>
			</tr>
			<tr>
			...
			<tr>
				<td colspan="2" style="font-size: 20px; padding: 30px 15px 0 15px;">Items
					Detail</td>
			</tr>
			<tr>
				<td colspan="2" style="padding: 15px;"><c:forEach var="p"
						items="${order.orderdetailses }">
						<p
							style="font-size: 14px; margin: 0; padding: 10px; border: solid 1px #ddd; font-weight: bold;">
							<span
								style="display: block; font-size: 13px; font-weight: normal;">${p.product.name}
								x ${p.quantity}</span> Total: $${p.amount} <b
								style="font-size: 12px; font-weight: 300;">[
								${p.product.unitPrice} / ${p.product.unitBrief} ]</b>
						</p>
					</c:forEach></td>
			</tr>
		</tbody>
		<tfooter>
		<tr>
			<td colspan="2" style="font-size: 14px; padding: 50px 15px 0 15px;"><strong
				style="display: block; margin: 0 0 10px 0;">Regards</strong> PTStore<br>
				Thong Tony, Pin/Zip - 710000, 24-26 Phan Liem, Sai Gon, Viet Nam<br>
				<br> <b>Phone:</b> 03552-222011<br> <b>Email:</b>
				contact@ptstore.vi</td>
		</tr>
		</tfooter>
	</table>
</div>
<c:if test="${order.status == false}">
	<a href="${pageContext.request.contextPath }/orders/cancel/${order.id}"
