...
<link rel="stylesheet" href="${pageContext.request.contextPath}/resources/vendor/css/ACE.css">


<c:if test="${pagedListHolder.pageCount > 1}">
	<ul class="pagination1 model-1">
		<c:if test="${!pagedListHolder.firstPage}">
			<li ><a class="prev"
				href="<%=StringUtils.replace(pagedLink, "~", String.valueOf(pagedListHolder.getPage() - 1))%>">&laquo;</a></li>
		</c:if>
		...
		<c:if test="${!pagedListHolder.lastPage}">
			<li ><a class="next"
				href="<%=StringUtils.replace(pagedLink, "~", String.valueOf(pagedListHolder.getPage() + 1))%>">&raquo;</a></li>
		</c:if>
	</ul>
</c:if>
